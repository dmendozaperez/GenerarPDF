...
<xsl:stylesheet
  ...
  <xsl:variable name="cantidadItemsDetalle" select="count(/r:Invoice/cac:InvoiceLine)" />
  ...
  <!-- ITEMS DE DETALLE -->
  <xsl:template name="ITEMS_DETALLE" match="cac:InvoiceLine">
    <div class="item">
      ID: <xsl:value-of select="cbc:ID"/>
      <br/>
      InvoicedQuantity: <xsl:value-of select="cbc:InvoicedQuantity"/>
      <br/>
