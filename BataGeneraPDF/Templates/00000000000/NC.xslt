...
<xsl:stylesheet
  ...
  <xsl:variable name="cantidadItemsDetalle" select="count(/r:CreditNote/cac:CreditNoteLine)" />
  ...
  <!-- ITEMS DE DETALLE -->
  <xsl:template name="ITEMS_DETALLE" match="cac:CreditNoteLine">
    <div class="item">
      ID: <xsl:value-of select="cbc:ID"/>
      <br/>
      CreditedQuantity: <xsl:value-of select="cbc:CreditedQuantity"/>
      <br/>
